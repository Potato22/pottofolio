...
function getDeviations(url,limit,start){var deviations=[],limit=limit||null,start=start||0;let xhr=new XMLHttpRequest;xhr.open("GET","https://backend.deviantart.com/rss.xml?q=gallery:Potato2292/78916640"),xhr.send(),console.log("request sent"),xhr.onload=function(){let json1=xml2json((new DOMParser).parseFromString(xhr.response,"text/xml")),json2,items=JSON.parse("{"+json1.slice(11)).rss.channel.item;for(var i=0,l=items.length;i<l;i++)if(!(i<start)){if(limit&&i==start+limit)break;var object={};object.title=items[i].title,object.link=items[i].link,object.image=items[i]["media:content"]["@url"],deviations.push(object),console.log("feed received")}console.log("DATA:"),processDeviations(deviations),deviantARTGalleryPlugin("Potato2292","78916640","16:9")}}
...
function deviantARTGalleryPlugin(e, t, n) {
    var s = [];
    ! function () {
        console.log("fetching user:", e, "folder:", t, "");
        var a = "https://backend.deviantart.com/rss.xml?q=gallery:Potato2292/78916640:" + e + "/" + t;
        window.callback = function (e) {
            for (var t = e.items, a = 0, i = t.length; a < i; a++) {
                var l = {};
                l.title = t[a].title, l.image = t[a].enclosure.link, s.push(l)
            }
            for (var d = "", a = 0, i = s.length; a < i; a++) d += '<img data-aos="zoom-in" class="imgs cards imgcard daImgs" src="' + s[a].image + '"/>';
            document.getElementById("daImg").innerHTML = d, console.log("images fetched.\n", e.feed.title, "\n", e.status, "\n\n Plugins by jamesl1001\nhttps://github.com/jamesl1001/deviantART-Gallery-Plugin\nhttps://github.com/jamesl1001/deviantART-API\n\nThis version was heavily modified by Potto.");
            const event = new Event("finish");
            document.getElementById("daImg").dispatchEvent(event)
        };
